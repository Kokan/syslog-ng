...
#undef assert_msg_field_equals
#define assert_msg_field_equals(msg, name, expected, expected_value_len) \
  do { \
   gssize value_len = -1; \
   const gchar *actual = log_msg_get_value_by_name(msg, name, &value_len); \
   \
   gssize exp_value_len = expected_value_len; \
   if (exp_value_len < 0) \
     exp_value_len = strlen(expected); \
   \
   cr_assert_eq(value_len, exp_value_len, "Message name(%s) value_len(%ld) expected_value_len(%ld) at line %d", name, value_len, exp_value_len, __LINE__); \
   cr_assert_str_eq(actual, expected, "Message name(%s) actual(%s) expected(%s) at line %d", name, actual, expected, __LINE__); \
  } while(0)
...
Test(rewrite, condition_success)
{
  ...
  assert_msg_field_equals(msg, "device_id", "00100", -1);
...
Test(rewrite, subst_pcre_unused_subpattern)
{
  ...
  assert_msg_field_equals(msg, "0", "abc", -1);
  assert_msg_field_equals(msg, "1", "a", -1);
  assert_msg_field_equals(msg, "2", "", -1);
  assert_msg_field_equals(msg, "3", "bc", -1);
  rewrite_teardown(msg);
}
...
Test(rewrite, set_field_exist_and_group_set_multiple_fields_with_glob_pattern_literal_string)
{
  ...
  invoke_rewrite_rule(test_rewrite, msg);
  assert_msg_field_equals(msg, "field.name1", "value", -1);
  assert_msg_field_equals(msg, "field.name2", "value", -1);
  rewrite_teardown(msg);
}
...
Test(rewrite, set_field_exist_and_group_set_multiple_fields_with_glob_question_mark_pattern_literal_string)
{
  ...
  invoke_rewrite_rule(test_rewrite, msg);
  assert_msg_field_equals(msg, "field1", "value", -1);
  assert_msg_field_equals(msg, "field2", "value", -1);
  rewrite_teardown(msg);
}
...
Test(rewrite, set_field_exist_and_group_set_template_string_with_old_value)
{
  ...
  invoke_rewrite_rule(test_rewrite, msg);
  assert_msg_field_equals(msg, "field1", "value alma", -1);
  rewrite_teardown(msg);
}
...
Test(rewrite, unset_field_disappears)
{
  ...
  invoke_rewrite_rule(test_rewrite, msg);
  assert_msg_field_equals(msg, "field1", "", -1);
  assert_msg_field_equals(msg, "PROGRAM", "foobar", -1);
  rewrite_teardown(msg);
}
